...
export const EventCard: React.FC<EventCardProps> = ({ 
  id, title, date, location, price, currency, images, category, organization, isTrending
}) => {
      ...
      <div className="p-5 flex flex-col flex-grow relative bg-surface z-20">
        ...
        <div className="mt-auto pt-5 mt-5 flex justify-between items-center border-t border-border/60">
          <span className="font-extrabold text-text-primary text-xl tracking-tight">
            {price === 'Free' || price === 0 ? 'Free' : (typeof price === 'string' && price.includes('-') ? `${currency === 'INR' ? '₹' : currency === 'EUR' ? '€' : currency === 'GBP' ? '£' : '$'}${price.split(' - ')[0]} - ${currency === 'INR' ? '₹' : currency === 'EUR' ? '€' : currency === 'GBP' ? '£' : '$'}${price.split(' - ')[1]}` : `${currency === 'INR' ? '₹' : currency === 'EUR' ? '€' : currency === 'GBP' ? '£' : '$'}${price}`)}
          </span>
