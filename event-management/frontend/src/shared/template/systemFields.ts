...
export const SYSTEM_FIELDS: FieldSpec[] = [
  ...
  { key: 'isPaid', label: 'Paid Event', fieldType: 'toggle', required: false, defaultValue: 'false', section: 'basics', order: 5, form: 'Basic Info', category: 'Event Details', formOrder: 0, categoryOrder: 0 },
  ...
  { key: 'timezone', label: 'Timezone', fieldType: 'text', required: false, defaultValue: 'Asia/Kolkata', section: 'datetime', order: 14, form: 'Basic Info', category: 'Date & Time', formOrder: 0, categoryOrder: 1 },
...
export function mergeSystemFields(fields: FieldSpec[]): FieldSpec[] {
  const filteredFields = fields.filter(f => f.key !== 'isFree');
  const byKey = new Map(filteredFields.map((f) => [f.key, withDefaultLayout(f)] as const));
